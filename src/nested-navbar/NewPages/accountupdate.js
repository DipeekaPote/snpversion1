...
const Accountupdate = ({ onClose, selectedAccount }) => {
    ...
    useEffect(() => {
        if (selectedAccount) {
            ...
            setSelectedUser(mappedTeamMembers.filter((member) =>
                selectedAccount.teamMember.some((selected) => selected._id === member.value)
            ));

            // Map Tags
            const mappedTags = selectedAccount.tags?.map((tag) => ({
                value: tag._id,
                label: tag.tagName,
                colour: tag.tagColour,
                customTagStyle: {
                                    ...
                                    <Box mt={2}>
                                        ...
                                        <Autocomplete
                                            ...
                                            value={options.filter((option) =>
                                                selectedUser.some((selected) => selected.value === option.value)
                                            )}
