...
import {
  Checkbox, Autocomplete, Switch, FormControlLabel, Box, Button, Drawer, Typography,
  Chip, IconButton, Divider, Select, MenuItem, InputLabel, TextField, FormControl, FormLabel, InputAdornment, Popover, ListItem, List, ListItemText, Table, TableBody,
...
const Invoice = () => {
  ...
  const fetchInvoices = async (data) => {
    try {
      ...
      fetch(`http://127.0.0.1:7650/workflow/invoices/invoice/invoicelistby/accountid/${data}`, requestOptions)
  ...
  const handleDuplicate = async (_id) => {
    ...
    try {
      ...
      const response = await fetch(`http://127.0.0.1:7650/workflow/invoices/invoice`, requestOptions);
      const result = await response.json();
      console.log(result)
      if (result.message === "Invoice created successfully") {
        toast.success("Invoice duplicated successfully");
        fetchInvoices(); // Refresh the list after duplication
      } else {
  ...
  return (
    <Box sx={{ mt: 2 }}>
      <Button variant="contained" onClick={handleCreateInvoiceClick} sx={{ mb: 3 }}>New Invoice</Button>
      <Paper>
        <Table sx={{ minWidth: 650 }} aria-label="simple table">
          <TableHead>
            <TableRow>
              ...
              <TableCell><strong>Amount P...</strong></TableCell>
              <TableCell><strong>Balance D...</strong></TableCell>
              <TableCell><strong>Last PAI..</strong></TableCell>
              <TableCell><strong>Description</strong></TableCell>
              <TableCell><strong>Locked D...</strong></TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
                ...
                <TableCell sx={{ textAlign: 'end' }}>
                  <IconButton onClick={() => toggleMenu(row._id)} style={{ color: '#2c59fa' }}>
                    ...
                    {openMenuId === row._id && (
                      ...
                      >
                        <Typography sx={{ fontSize: '12px', fontWeight: 'bold' }} onClick={() => handleEdit(row._id)}>Edit</Typography>
